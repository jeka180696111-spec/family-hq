    ...
    async def tick(self) -> None:
        ...
        try:
            ...
            if available_scenes or climate_info:
                phase_rules = ""
                if ev["kind"] == "start":
                    # Матвей ТОЛЬКО ЧТО УСНУЛ — свет ВЫКЛ полностью
                    phase_rules = (
                        "СЕЙЧАС ФАЗА: МАЛЫШ УСНУЛ.\n"
                        "→ Свет надо ВЫКЛЮЧИТЬ полностью (не приглушить, а off).\n"
                        "→ Кондер только если климат вне нормы, иначе не трогай.\n"
                        "→ Приглушение делать НЕ надо — уже поздно, он уснул.\n\n"
                        "Примеры делегаций:\n"
                        "  «Дворецкий, малыш уснул — запусти сцену «Спальня выкл».»\n"
                        "  «Дворецкий, 26°C жарко — запусти сцену «Кондер 24 авто».»\n"
                    )
                else:  # end / wake
                    phase_rules = (
                        "СЕЙЧАС ФАЗА: МАЛЫШ ПРОСНУЛСЯ.\n"
                        "→ Свет надо ВКЛЮЧИТЬ (мягкий утренний или яркий днём).\n"
                        "→ Днём (после 09:00) — яркий белый, вечером — приглушённый.\n"
                        "→ Кондер только если климат вне нормы.\n\n"
                        "Примеры делегаций:\n"
                        "  «Дворецкий, малыш проснулся — запусти сцену «Спальня ярко».»\n"
                        "  «Дворецкий, вечер — запусти сцену «Спальня уют».»\n"
                    )

                climate_action_rule = (
                    "\n\n═══ ДЕЛЕГИРОВАНИЕ ДВОРЕЦКОМУ ═══\n"
                    "Ты — Няня, физику дома НЕ трогаешь руками. Действия со "
                    "светом, кондером, увлажнителем и т.п. делегируй Дворецкому "
                    "ОТДЕЛЬНОЙ строкой начинающейся со слова «Дворецкий,».\n\n"
                    f"{phase_rules}\n"
                    "ПРАВИЛА:\n"
                    "• «Дворецкий» — ПЕРВЫМ словом строки, иначе не поймёт.\n"
                    "• Если есть готовая СЦЕНА (список ниже) — проси её ДОСЛОВНЫМ "
                    "именем в «кавычках».\n"
                    "• Если сцены нет — попроси действие человеческим языком.\n"
                    "• НЕ выдумывай имена сцен которых нет в списке.\n"
                    "• Не делегируй если в этом нет смысла.\n"
                    "═══"
                )
