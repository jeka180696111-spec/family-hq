        ...
        try:
            ...
            if is_wake:
                ...
                prompt = (
                    ...
                    "соотносится с обычной для Матвея (не возрастной нормой).\n"
                    "• Контекст: что это значит в общей картине дня "
                    "(сколько уже спал суммарно, как прошла ночь, "
                    "повторяется ли паттерн).\n"
                    "• Действие: следующий конкретный шаг — HH:MM "
                    "следующего сна, желаемая длительность, что важно "
                    "сделать до этого.\n\n"
                    "ТОН: тёплый, человечный, как старшая сестра-педиатр.\n"
                    "Без сюсюканья, без 🤱 💕 🥰. Не цитируй сообщение "
                    "Марины. Не пиши «отлично выспался» если короткий сон. "
                    "Длительности — в формате Хч YYм (не «118 мин»).\n"
                    "Длина: 4-8 строк. Структура важнее краткости."
                )
            else:
                prompt = (
                    f"Маринa только что внесла: «{ev['event']}» в "
                    ...
                    "Когда планируется пробуждение (HH:MM). Если "
                    "уложили близко к плану — отметь, если сдвиг — "
                    "тоже честно. Тёплый тон, без сюсюканья. "
                    "Длительности в формате Хч YYм, не «X мин»."
                )
            try:
