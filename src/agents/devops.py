...
class DevOpsAgent(BaseAgent):
    ...
    async def _try_fast_path(self, message_text: str) -> str | None:
        ...
        # === НЕ ВХОДИМ В FAST-PATH если явно просят создать/удалить
        # автоматизацию/правило — иначе имя сцены внутри условия («если
        # все ушли то выкл везде свет») случайно матчит существующую
        # сцену «Свет везде выкл» и мы её запускаем вместо создания
        # правила. Пусть LLM разбирает такие фразы сама через инструменты
        # automation_add / automation_delete.
        AUTOMATION_INTENT = (
            "создай автоматизац", "создать автоматизац",
            "новая автоматизац", "новое правило", "создай правило", "создать правило",
            "добавь автоматизац", "добавить автоматизац",
            "удали автоматизац", "удалить автоматизац", "удали правило",
            "выключи автоматизац", "включи автоматизац",
        )
        if any(hint in low for hint in AUTOMATION_INTENT):
            return None
