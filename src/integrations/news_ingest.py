...
_ALERT_CLEAR_PATTERNS = re.compile(
    ...
    r"минула\s+загроз|минул(а|о)\s+небезпек|"
    r"загрози\s+немає|загроза\s+відсутн|"
    r"загроза\s+мин|загроза\s+більше\s+не|"
    r"вс[ёе]\s+чист(о|іше)|небо\s+чист|"
    r"чист[оа]я?\s+небо|"
    r"безпечно\s+в\s+регіон|можна\s+повертат|"
    r"alert\s+(over|cleared|cancelled)|"
    r"✅.*тривог|✅.*тревог|✅.*чист|✅.*відб"
    r")",
    re.IGNORECASE | re.UNICODE,
)
...
_THREAT_PATTERNS = re.compile(
    r"("
    r"\bшахед\w*|шахіб\w*|герань\w*|гераниум\w*|geran\w*|"
    r"\bкалібр\w*|калибр\w*|"
    r"\bкрилат[аи]?\s+ракет\w*|балістичн[аи]?\s+ракет\w*|балістика\b|"
    r"\bракет\w*|"                                       # любая «ракета»
    r"\bреактивн\w*|"                                    # «реактивные [летят]» — сленг для ракет
    r"\bсмерч\w*|\bград\b|\bградов\w*|\bураган\w*|"      # РСЗО-системы
    r"\bзапуск\s+ракет\w*|загроза\s+удару|"
    r"\bвибух\w*|взрыв\w*|"
    r"\bобстріл\w*|обстрел\w*|"
    r"\bбпла\b|\bдрон\w*|\bмиг\b|\bтуp\w*|"
    r"\bсу-\d+|\bсу\d+|"                                 # Су-24/34/35 (штурмовики/бомбардировщики)
    r"\bпуск\w*|"
    r"\bпідл[іi]т\w*|подл[её]т\w*|"                     # подлёт/підліт
    r"\bкурс(ом)?\s+на\s+|курс(ом)?\s+в\s+сторону|"     # «курсом на Одессу»
    ...
    r"\bоникс\w*|онікс\w*|"                             # Оникс P-800 (противокорабельная)
    r"\bциркон\w*|цирко[нh]\w*|"                        # 3М22 Циркон
    r"\bх-\d+|\bх\d+|"                                   # Х-101, Х-59
    r"\bкасетн\w*|"                                      # кассетные
    r"\bгруп[аи]\s+дрон|рой\s+дрон|"
    r"\bу\s+укритт[яi]|в\s+укрыти|тревожно|тривожно|"   # призывы в укрытие = тоже угроза
    r"\bзатока|каролино|сергеевк|черноморск|іллічівськ|"  # локальные точки Одес. обл — как маркер
    r"овідіополь|овидиополь|южн[іо]й|аркад[иі][яi]"
    r")",
    re.IGNORECASE | re.UNICODE,
)
