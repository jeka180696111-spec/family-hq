...
Тебе дан список постов из Telegram-каналов о ТЕКУЩЕЙ ВОЗДУШНОЙ ТРЕВОГЕ в Одесской области.

⚠️ КРИТИЧНО — АНТИ-ГАЛЛЮЦИНАЦИИ:
• В `weapons` включай ТОЛЬКО то что летит В НАШ РЕГИОН (Одесская обл).
• Если пост говорит «Николаевская область — 3 шахеда» — это НЕ наши шахеды, идёт в other_regions.
• Если пост говорит «Ту-95 взлетели с Энгельса» БЕЗ упоминания направления в Одесскую — это НЕ наше, игнорируй.
• Если пост говорит «по всей Украине воздушная тревога» без деталей — НЕ добавляй ничего.
• ЛУЧШЕ выдать пустой weapons: [], чем выдумать что не было явно направлено к нам.
• Если посты упоминают ТОЛЬКО «бандероли» — weapons должен содержать ТОЛЬКО КАБ. Не добавляй шахеды/ракеты «по умолчанию».
• Мониторинг-каналы (Николаевский Ванёк, повітряна тривога и т.д.) часто пишут про всю Украину — фильтруй только upcoming for Одесская обл.

ГЛАВНОЕ ПРАВИЛО АГРЕГАЦИИ: одно и то же оружие → ОДИН элемент weapons.
...
• «Оникс», «Онікс», «Onyx», «П-800» = Оникс (противокорабельная, часто из Крыма)
• «Циркон», «3M22 Zircon» = Циркон
• «КАБ», «ФАБ», «УМПБ», «управляемая авиабомба» = соответствующая КАБ/ФАБ/УМПБ

Триггеры угрозы (даже без явного типа) — тревожные слова которые означают что что-то летит:
«пуск», «пуски», «подлёт», «підліт», «курсом на», «летит в сторону», «в укрытие/укритті» —
не игнорируй пост даже если тип оружия не назван, добавляй его в контекст targets/eta.
...
ПРИЛЁТЫ (hits) — ловим АКТИВНО. Пропустить прилёт = провалить работу.
Ищи в постах любое из:
- явные слова: «прилёт», «прилет», «удар по», «попадання», «попадание»
- шум: «ВЗРЫВЫ», «ВИБУХИ», «взрыв слышен», «вибух чути», «слышны взрывы»,
  «💥» (эмодзи взрыва, особенно повторяющиеся 💥💥💥)
- последствия: «уражено», «поражено», «загорілось», «загорелось»,
  «детонація», «детонация», «дим над», «дым над», «пожежа», «пожар»,
  «тупо по домам», «прямое попадание», «прямой удар»
- «Новых целей нет» / «отбой ракетам» после взрывов = подтверждение прилёта
- Фото/видео с описанием разрушений — если в тексте намёк на удар

Даже ОДНО упоминание любого из выше = ОБЯЗАТЕЛЬНО добавляй в hits (confirmed: false).
Если 2+ канала или официалы (МОУ, ОДА) → confirmed: true.
Если пост содержит просто «💥💥💥 ВЗРЫВЫ» без деталей — hits: [{{"location": "неизв. район", "detail": "взрывы", "confirmed": false}}].
Никогда не ставь hits: [] если в постах есть хоть намёк на взрыв/удар.
...
class AlertDigestManager:
    ...
    async def refresh_digest(self, region: str) -> None:
        ...
        # 2. Собрать посты по этому региону — только ПОСЛЕ начала тревоги.
        #    Иначе LLM тащит мусор из предыдущих алертов (шахеды в 3 ночи
        #    попадали в digest 8-утренней тревоги).
        started_iso = aa.started_at or ""
        async with self._memory._engine.connect() as conn:
            posts = list(await conn.execute(
                select(AlertPost).where(
                    (AlertPost.region == region)
                    & (AlertPost.received_at >= started_iso)
                )
        ...
        # 3. Сформировать текст для LLM — последние 40, свежие сверху
        posts_lines = []
        for p in posts[:40]:
