...
ПРАВИЛА:
1. Извлеки СТРОГО ТО ЧТО ЯВНО НАПИСАНО в постах. НЕ ВЫДУМЫВАЙ.
2. Если данных нет — используй null или пустой массив [].
3. Если один шахед упоминается в 5 каналах — это ОДИН шахед, не 5.
4. Веры больше самому свежему посту (у каждого timestamp).
5. Если в постах цифра «~20», а потом «уточнили: 15» — бери 15.
...
{{
  ...
  "weapons": [
    ...
    {{"type": "Х-101", "count": 3, "origin": "Ту-95 Энгельс"}},
    {{"type": "Искандер", "count": 2, "origin": "Крым"}}
  ],
  "targets": ["Юг Одесской обл", "Овидиополь", "порт Одессы"],
  "eta": [
    {{"weapon": "Шахед", "arrival_time": "~21:47"}},
    {{"weapon": "Ракеты", "arrival_time": "~21:56"}}
  ],
  "hits": [
...
Регион по умолчанию — Одесская обл. Если параллельная тревога в другом регионе — добавляй в `other_regions`.

ВСЕГДА возвращай валидный JSON. НИКАКОГО дополнительного текста.
"""
