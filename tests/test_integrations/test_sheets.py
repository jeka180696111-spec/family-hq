...
async def test_append_baby_diary_calls_run_sync(sheets_client):
    ...
    with patch.object(sheets_client, "_get_client", new_callable=AsyncMock) as mock_gc, \
         patch.object(sheets_client, "_run_sync", new_callable=AsyncMock) as mock_sync:

        mock_sync.return_value = None

        # Mock _open_worksheet
        with patch.object(sheets_client, "_open_worksheet" if hasattr(sheets_client, "_open_worksheet") else "_get_client", new_callable=AsyncMock) as _:
            # Patch _run_sync to return a mock worksheet
